...
test("Media Review UI uses the transient session boundary without changing document persistence", async () => {
  ...
  assert.match(index, /id="media-review-skip-back"[^>]*aria-label="Назад на 5 секунд"[^>]*title="Назад на 5 секунд"/);
  assert.match(index, /id="media-review-play" class="icon-button secondary-button"[^>]*aria-label="Воспроизвести"[^>]*title="Воспроизвести"/);
  assert.match(index, /id="media-review-skip-forward"[^>]*aria-label="Вперёд на 5 секунд"[^>]*title="Вперёд на 5 секунд"/);
  assert.match(index, /id="disable-media-review" class="icon-button secondary-button"[^>]*aria-label="Закрыть проверку по записи"[^>]*title="Закрыть проверку по записи"/);
  ...
  assert.match(renderer, /function seekMediaReviewBy\(seconds\) \{[\s\S]*Math\.min\(duration, Math\.max\(0, currentTime \+ seconds\)\)[\s\S]*mediaReviewSession\?\.seek/);
  assert.match(renderer, /function canUseMediaReviewShortcut\(event\) \{[\s\S]*mediaReviewSnapshot\.enabled[\s\S]*isEditableOrFormControl/);
  assert.match(renderer, /event\.code === "Space"[\s\S]*toggleMediaReviewPlayback\(\)/);
  assert.match(renderer, /event\.code === "ArrowLeft" \|\| event\.code === "ArrowRight"[\s\S]*seekMediaReviewBy/);
  assert.match(renderer, /mediaReviewSession\?\.togglePlayback\(\)/);
  assert.match(renderer, /const playbackIconName = isPlaying \? "Pause" : "Play";[\s\S]*if \(mediaReviewPlayButton\.dataset\.iconName !== playbackIconName\) \{[\s\S]*mediaReviewPlayButton\.replaceChildren\(createIcon\(playbackIconName\)\);[\s\S]*mediaReviewPlayButton\.dataset\.iconName = playbackIconName;/);
  ...
  assert.match(renderer, /mediaReviewHeader\.parentElement !== mediaPaneHeaderSlot[\s\S]*mediaPaneHeaderSlot\.append\(mediaReviewHeader, mediaReviewStatus\);[\s\S]*mediaReviewControls\.parentElement !== mediaPaneTransportSlot[\s\S]*mediaPaneTransportSlot\.append\(mediaReviewControls, relinkMediaSourceButton\)/);
  assert.match(renderer, /mediaReviewHeader\.parentElement !== mediaTransportSlot[\s\S]*mediaTransportSlot\.append\(mediaReviewHeader, mediaReviewStatus, mediaReviewControls, relinkMediaSourceButton\)/);
  assert.doesNotMatch(renderer, /addEventListener\("resize"/);
  assert.match(session, /function createSeekResolver/);
  assert.match(session, /togglePlayback: \(\) => enabled[\s\S]*mediaController\.togglePlayback\(\)/);
