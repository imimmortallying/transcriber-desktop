...
вручную: приложение открывает папку с готовым одним `.txt`-файлом, который
пользователь или посредник прикладывает к письму `imimmortallyingwork@yandex.ru`.
...
- **Recognition Support Report** создаётся при неудачном запуске или выполнении
  распознавания. Renderer показывает краткую ошибку в отдельном диалоге и
  кнопку «Открыть папку с отчётом», а не Python traceback.
