...
| Clean Full Setup, versioned Client | No ASR installation; run current Full Setup. | Creates `<root>/Clients/<version>` and sibling `<root>/Runtime`; Client is not installed at root. | Static packaging assertions. | **Validated** before Slice 6C. | Slice 6C Full Setup does not deploy Coordinator and is not release-ready. |
| Root-level stable launcher | Successful Full Setup. | `<root>/asr-launch.exe` exists outside versioned Client. | Static packaging assertion; launcher is compiled in `npm run dist:win`. | **Validated** before Slice 6C. | Full Setup does not deploy Coordinator, so this launcher reports `20` until Slice 6D. |
| Desktop shortcut launch path | Successful Full Setup; open Desktop shortcut. | Shortcut targets root launcher. | Shortcut target: static only. | **Validated** before Slice 6C. | Current Full Setup launcher reports `20` because Coordinator deployment is absent. |
| Start Menu shortcut launch path | Successful Full Setup; open Start Menu shortcut. | Shortcut targets root launcher. | Shortcut target: static only. | **Validated** before Slice 6C. | Current Full Setup launcher reports `20` because Coordinator deployment is absent. |
...
| Standalone Coordinator readonly state launch | Run Coordinator from valid `<root>/Coordinator/asr-coordinator.exe` geometry with controlled state results. | Coordinator maps state and selected Client outcomes to stable exit codes; launches only validated `activeClient`, and success means confirmed process creation rather than READY/health. | **Behavioral:** `npm run test:coordinator`. | Not manually performed on a built Coordinator. | Root launcher now delegates to Coordinator, but Full Setup does not deploy it yet. |
| SEA Coordinator state integration | Build SEA Coordinator, copy it to temporary valid Coordinator geometry, and provide state selecting either a missing Client or normal `0.2.0` Client beside an unused `0.1.0` directory. | Missing Client exits `16` (`SELECTED_CLIENT_UNAVAILABLE`). A normal non-elevated NSIS Client is created only from selected `0.2.0`, writes its marker and gives Coordinator exit `0`; both state-slot byte sequences remain unchanged. | **Behavioral:** `npm run test:coordinator-sea`. | **Validated** in an isolated Windows layout: real root launcher invoked real SEA Coordinator; real state selected `0.2.0` despite an additional `0.1.0` directory; the normal selected Client created `CLIENT_LAUNCHED.txt` and launcher exited `0`. Earlier exit `18` came from a temporary `requireAdministrator` fixture, not a Coordinator defect. | Test excludes normal Node PATH and now covers real normal-Client process creation; it does not validate READY or deployment. Console flash was observed separately and remains unresolved. |
| Stable launcher Coordinator handoff | Run built root launcher with a controlled Coordinator target and arbitrary `Clients` directories. | Launcher validates only fixed `<root>/Coordinator/asr-coordinator.exe`, rejects missing/unsafe/unstartable target with `20`/`21`/`22`, preserves Coordinator `0` and `10`–`19`, and maps unknown Coordinator non-zero to `23`. | **Behavioral:** `npm run test:launcher`. | **Validated** in the isolated real-launch layout: root launcher invoked real SEA Coordinator, which launched selected `0.2.0` Client and returned `0`. Console flash was observed but remains unresolved. | Full Setup does not deploy Coordinator in Slice 6C; a new Full Setup launcher therefore reports `20` until Slice 6D. |
...
| active / candidate / known-good launch selection | **Future / undefined implementation.** | Root launcher delegates to standalone Coordinator, which can read `activeClient` and create its process from validated geometry. Coordinator is not deployed by Full Setup, and candidate/activation/known-good fallback lifecycle is not implemented. |
