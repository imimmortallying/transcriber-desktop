...
| Silent Full Setup geometry initialization | No ASR installation; run Full Setup with `/S /currentuser /D=<root>`. | Before `customInstall`, silent flow derives the same root and `Clients/<version>` geometry as the interactive Install Files flow. | **Static:** `npm run test:packaging` checks the shared initialization function, its interactive callback, and its silent-flow call. | Not yet validated as a complete built-Setup clean installation. | The disposable tiny-Runtime smoke reached root-owned Runtime, Coordinator and launcher deployment, but did not complete late Client state provisioning; it is not full Setup lifecycle evidence. |
...
| Manual uninstall | Current registered versioned Client; add `<root>/DO_NOT_DELETE.txt`, then perform ordinary version-owned uninstall. | Removes Client, InstallationState, Runtime, Runtime staging/previous, known Coordinator protocol files and root launcher after structural ownership validation; user data/results remain. | Static packaging assertion. | **Validated** before Coordinator deployment: root contained only `DO_NOT_DELETE.txt` afterward. | Built-Setup Coordinator cleanup remains to be exercised; foreign residue prevents recursive Coordinator/root deletion. |
