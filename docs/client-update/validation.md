...
| active / candidate / known-good launch selection | **Future / undefined implementation.** | Standalone Coordinator can read `activeClient` and create its process from validated geometry, but current NSIS launcher does not consume state, Coordinator is not deployed by Full Setup, and candidate/activation/known-good fallback lifecycle is not implemented. |
