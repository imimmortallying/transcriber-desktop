...
| SEA Coordinator state integration | Build SEA Coordinator, copy it to temporary valid Coordinator geometry, and provide valid state selecting a missing Client. | EXE executes shared state code, exits `16` (`SELECTED_CLIENT_UNAVAILABLE`), emits no output and leaves state bytes unchanged. | **Behavioral:** `npm run test:coordinator-sea`. | Not manually performed on a built Full Setup or production launch path. | Test excludes normal Node PATH; it does not validate Client success, READY or deployment. |
| Stable launcher Coordinator handoff | Run built root launcher with a controlled Coordinator target and arbitrary `Clients` directories. | Launcher validates only fixed `<root>/Coordinator/asr-coordinator.exe`, rejects missing/unsafe/unstartable target with `20`/`21`/`22`, preserves Coordinator `0` and `10`–`19`, and maps unknown Coordinator non-zero to `23`. | **Behavioral:** `npm run test:launcher`. | Isolated real-SEA handoff and console-window observation were not performed. | Full Setup does not deploy Coordinator in Slice 6C; a new Full Setup launcher therefore reports `20` until Slice 6D. |
